...
RegisterEvents()
{
	dhAddClientHook(CHK_TakeDamage, FT_PrehookTakeDamage);
...
*/
public Action:FT_PrehookTakeDamage(victim, attacker, inflictor, Float:dmg, &Float:dmgMultiplier, dmgType) //done
{
	if (GetConVarBool(g_cvDebug))
	...
	if (attacker != victim) 
	{
		...
		if (g_bFrozen[victim] && !g_bIsHumiliationRound)
		{
			...
			if (multiplier > 0.0)
			{
				...
				new adjustedHealth = GetClientHealth(victim) + RoundToCeil(dmg * dmgMultiplier * multiplier);
